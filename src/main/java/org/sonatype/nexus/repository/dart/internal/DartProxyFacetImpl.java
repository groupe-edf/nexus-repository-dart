 ...
 */
@Named
public class DartProxyFacetImpl
    extends ProxyFacetSupport
{
  ...
  private final DartJsonProcessor composerJsonProcessor;

  @Inject
  public DartProxyFacetImpl(final DartJsonProcessor composerJsonProcessor) {
  ...
  private String getZipballUrl(final Context context) {
    try {
      ...
      Request request = new Request.Builder().action(GET).path("/" + buildProviderPath(vendor, project))
          .attribute(DartProviderHandler.DO_NOT_REWRITE, "true").build();
      Response response = getRepository().facet(ViewFacet.class).dispatch(request, context);
